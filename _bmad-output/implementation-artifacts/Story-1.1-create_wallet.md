...
- [ ] **Task 2: Implement Core Wallet Creation Logic (AC: 3, 4)**
  - [ ] Subtask 2.1: Implement BIP39 mnemonic generation for recovery passphrase in `src/wallet_manager.rs`. (FR2, FR7)
  - [ ] Subtask 2.2: Implement secure storage of wallet name and encrypted passphrase into SQLite via `src/db.rs`. (FR3, FR9, FR11, NFR5)
  - [ ] Subtask 2.3: Define `Wallet` and `Account` data structures in `src/models.rs`.
- [ ] **Task 3: Expose `create_wallet` as MCP Tool (AC: 1, 2)**
  - [ ] Subtask 3.1: Define the `create_wallet` tool with `snake_case` naming and `camelCase` JSON fields as per API design in `src/api_handlers.rs` (utilizing `rmcp` and `schemars`). (FR12, FR15)
  - [ ] Subtask 3.2: Create the Axum handler for the `/mcp` route in `src/main.rs` (or `src/api_handlers.rs`) to dispatch MCP requests.
  - [ ] Subtask 3.3: Ensure robust input validation is implemented at the API boundary. (NFR9)
