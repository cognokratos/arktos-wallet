...
async fn test_create_api_key() {
    ...
    let result = key_store.create(client_name).await;
...
async fn test_validate_api_key_valid() {
    ...
    let result = key_store.validate(&api_key).await;
...
async fn test_validate_api_key_invalid() {
    ...
    let result = key_store.validate("invalid_key_12345678").await;
...
async fn test_validate_api_key_revoked() {
    ...
    // Revoke the key
    let revoke_result = key_store.revoke(api_key.id).await;
    assert!(revoke_result.is_ok(), "Should revoke API key");

    // Try to validate revoked key
    let validation_result = key_store.validate(&x_api_key).await;
...
async fn test_list_api_keys_for_wallet() {
    ...
    let result = key_store.list().await;
...
async fn test_api_key_stored_hashed() {
    ...
    let validation = key_store.validate(&api_key).await;
    assert!(
        validation.is_ok(),
    ...
    let invalid_validation = key_store.validate(&format!("wrong_{}", api_key)).await;
